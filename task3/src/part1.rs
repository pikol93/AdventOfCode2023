...
pub fn part1(input_text: &str) {
    ...
    let points_adjacent_to_symbols = engine_schematic.symbols
        .iter()
        .flat_map(|symbol| {
            SYMBOL_NEIGHBOUR_OFFSETS.iter()
                .map(|(offset_x, offset_y)| (offset_x + symbol.char_index as i32, offset_y + symbol.line as i32))
                .map(|(point_x, point_y)| (point_x as usize, point_y as usize))
        })
        .collect::<Vec<_>>();

    let sum = engine_schematic.part_numbers
        .iter()
        .filter(|part_number| {
            points_adjacent_to_symbols.iter()
